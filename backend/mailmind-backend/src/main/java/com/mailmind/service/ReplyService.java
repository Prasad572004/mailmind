...
//
//        // Use plain text body if available, fall back to full body
...
//        }
//
//        // Call AIService — reuses existing generateSmartReplies method
...
//
//    // ─────────────────────────────────────────────────────────
//    // Send reply via Gmail API and mark email as replied
//    // ─────────────────────────────────────────────────────────
//    public void sendReply(String emailId, String replyBody, String userEmail) throws Exception {
//        User user = userRepository.findByEmail(userEmail)
//                .orElseThrow(() -> new RuntimeException("User not found"));
//
...
//                user.getEmail(),        // From: logged-in user
//                email.getFrom(),        // To: original sender
//                replySubject,
//                replyBody,
//                email.getThreadId()     // For threading
//        );
//
//        // Send via Gmail API
//        sendViaGmailApi(accessToken, rawMessage, email.getThreadId());
//
//        // Mark original email as replied in DB
//        email.setIsReplied(true);
//        email.setRepliedAt(LocalDateTime.now());
//        emailRepository.save(email);
//
//        log.info("Reply sent successfully for email: {}", emailId);
//    }
//
//    // ─────────────────────────────────────────────────────────
//    // Build RFC 2822 raw MIME message (required by Gmail API)
//    // ─────────────────────────────────────────────────────────
//    private String buildRawMimeMessage(String from, String to,
//                                        String subject, String body,
//                                        String threadId) {
//        // Strip name from "Name <email>" format to get clean To address
//        String toAddress = to;
//        if (to != null && to.contains("<") && to.contains(">")) {
//            toAddress = to.substring(to.indexOf("<") + 1, to.indexOf(">")).trim();
//        }
//
//        StringBuilder mime = new StringBuilder();
//        mime.append("From: ").append(from).append("\r\n");
//        mime.append("To: ").append(toAddress).append("\r\n");
...
//        mime.append(body);
//
//        // Base64 URL-safe encode (required by Gmail API)
//        return Base64.getUrlEncoder()
//                .withoutPadding()
//                .encodeToString(mime.toString().getBytes(java.nio.charset.StandardCharsets.UTF_8));
//    }
//
//    // ─────────────────────────────────────────────────────────
...
//            long expiresAt = user.getGmailTokenExpiresAt();
...
//
//    // ─────────────────────────────────────────────────────────
//    // Refresh token and persist new one to DB
//    // ─────────────────────────────────────────────────────────
